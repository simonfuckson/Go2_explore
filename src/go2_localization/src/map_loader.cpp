 ...
 */
int pubMap(const string &mapPath, const string &mapFrame, ros::Publisher &pub)
{
    ...
    cloudMsg.header.frame_id = mapFrame;
...
int main(int argc, char **argv)
{
    ...
    nh.param<string>("map_frame", mapFrame, "map");
    nh.param<string>("map_topic", mapTopic, "/map_cloud");

    auto mapPub = nh.advertise<sensor_msgs::PointCloud2>(mapTopic, 1, true);

    auto ret = pubMap(mapPath, mapFrame, mapPub);
    if (ret != 0) return ret;
